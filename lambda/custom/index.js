...
const buildClueResponse = (handlerInput) => {
  const cluePosition = session.get('cluePosition')
  const clue = skill.clues.get(cluePosition)
  ...
  const cardTitle = 'Clue number ' + clue.number
  const cardBody = clue.body
...
const GuessIntentHandler = {
  ...
  handle (handlerInput) {
    ...
    const shouldEnd = guess.status === 'correct'
